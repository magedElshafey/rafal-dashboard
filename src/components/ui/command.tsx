...
type CommandInputProps = React.ComponentProps<typeof CommandPrimitive.Input> & {
  ...
  surface?: 'embedded' | 'field'
}

function CommandInput({ className, wrapperClassName, surface = 'embedded', ...props }: CommandInputProps) {
  return (
    <div
      data-slot="command-input-wrapper"
      className={cn(
        'flex h-9 items-center gap-2 px-3',
        surface === 'embedded' && 'border-b',
        surface === 'field' && [
          'm-2 h-11 rounded-xl border border-black-50 bg-black-50',
    ...
    >
      ...
      <CommandPrimitive.Input
        ...
        className={cn(
          'flex h-10 w-full rounded-md bg-transparent py-3 text-sm text-content-primary outline-hidden',
          'placeholder:text-black-400',
          'disabled:cursor-not-allowed disabled:text-content-secondary disabled:opacity-100',
          className
        )}
        {...props}
      />
